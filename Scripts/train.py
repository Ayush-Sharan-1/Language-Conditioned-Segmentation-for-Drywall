"""
Training script for language-conditioned segmentation (supports both cracks and drywall).
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train language-conditioned segmentation model")
    
    # Dataset selection
    parser.add_argument("--dataset_type", type=str, default="cracks",
                        choices=["cracks", "drywall"],
                        help="Dataset type: 'cracks' or 'drywall' (default: 'cracks')")
    
    # Data paths
    parser.add_argument("--data_root", type=str, default=None,
                        help="Root directory containing train/valid splits (default: auto-set based on dataset_type)")
    ...
    # Set dataset-specific defaults based on dataset_type
    if args.data_root is None:
        if args.dataset_type == "cracks":
            args.data_root = "Dataset/cracks"
        elif args.dataset_type == "drywall":
            args.data_root = "Dataset/drywall-post"
        else:
            raise ValueError(f"Unknown dataset_type: {args.dataset_type}")
    
    # Determine class name based on dataset type
    if args.dataset_type == "cracks":
        class_name = "crack"
        dataset_display_name = "cracks"
    elif args.dataset_type == "drywall":
        class_name = "taping_area"
        dataset_display_name = "drywall"
    else:
        raise ValueError(f"Unknown dataset_type: {args.dataset_type}")
    ...
    run_name = f"{dataset_display_name}_attention_unet_v1_{timestamp}"
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_metrics, global_step = train_epoch(
            ...
            scaler, use_amp, class_name=class_name
        )
        ...
        if (epoch + 1) % args.val_every_n_epochs == 0:
            val_metrics = validate(
                ...
                prompt_embeddings_cache, use_amp, class_name=class_name
            )
